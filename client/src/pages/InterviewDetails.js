...
function InterviewDetails() {
  ...
  const [interviewersList, setInterviewersList] = useState([]); // Store list of available interviewers
  ...
  const fetchInterviewers = async () => {
    try {
      // Assumes backend route: router.get("/list-interviewers", ...) returns users with role="interviewer"
  ...
  useEffect(() => {
    fetchInterviews();
    fetchInterviewers(); // Load interviewers on mount
  }, []);
  ...
  const populateForm = (item) => {
    setForm({
      ...
      interviewerId: item.interviewerId || "", // Ensure ID is populated if editing
    });
  ...
  // --- Specific handler for Interviewer Dropdown ---
  const handleInterviewerChange = (e) => {
    ...
    setForm(prev => ({
      ...
      // Automatically set the name based on selection for display purposes
      ...
      <div className="card form-card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid-2">
            ...
            {/* --- REPLACED: Text Input with Dropdown for Interviewer --- */}
            <div className="form-field">
          ...
          <h3 className="section-label">Status & Configuration</h3>
          <div className="grid-2">
            <div className="form-field">
              <label>Status</label>
            ...
            <div className="form-field">
                {/* Invisible label pushes checkbox down to align with dropdown */}
                ...
                <div style={{display:'flex', alignItems:'center', gap: '10px', height: '45px'}}>
                    ...
                    Notify Hiring Manager on Update
                    </label>
                </div>
            </div>
          </div>
          
          {/* Evaluation Results - Enabled when Editing */}
          {id && (
            <>
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((it) => (
                <div className="table-row" key={it._id}>
                  ...
                  <span>
                      <span className={`status-pill ${it.status?.toLowerCase()}`}>{it.status || "Pending"}</span>
                  </span>
                  
                  <span>{it.result || "Pending"}</span>

                  <div className="row-actions">
                    <FaEdit className="icon" onClick={() => handleEdit(it)} title="Edit" />
                    <FaTrash className="icon delete" onClick={() => handleDeleteClick(it._id)} title="Delete" />
                  </div>
                </div>
              ))
            )}
