...
router.get("/", auth, async (req, res) => {
  try {
    ...
    // FIX: Send 'recruiter' as an alias for 'user' so the Recruiter frontend works
    res.json({ success: true, user: user, recruiter: user });
  } catch (error) {
...
router.put("/", auth, async (req, res) => {
  try {
    ...
    // --- DEBUG LOG: Check what data is actually reaching the server ---
    console.log("📥 Received Profile Update Data:", req.body);

    // Merge existing profile with new data
    // This ensures fields like companyWebsite (CamelCase) are merged correctly
    ...
    // FIX: Include 'recruiter' in the response
    res.json({ 
      success: true, 
      message: "Profile updated successfully", 
      user: updatedUser,
      recruiter: updatedUser 
    });
  } catch (error) {
